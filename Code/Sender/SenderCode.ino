...
// Muss zum "WLAN Kanal fuer Sender" vom Empfaenger passen.
#define ESPNOW_CHANNEL 11

// Muss zur "STA MAC fuer Sender" vom Empfaenger passen.
...
// Bei manchen Arduino-ESP32-Versionen heisst der erste Parameter wifi_tx_info_t.
// Wenn das bei dir nicht kompiliert, sag kurz Bescheid.
...
void loop() {
  ...
  if (!sleepMode && now - phaseStart >= MEASURE_TIME) {
    ...
    sendSensorPacket(true, true, SLEEP_TIME / 1000, avgPir, avgDistance, avgTilt, avgTemp, avgHum, avgLight);
    lastSendMs = now;
  }
  ...
  if (sleepMode) {
    ...
    if (now - lastSendMs >= SEND_INTERVAL) {
      sendSensorPacket(true, false, secondsLeft, avgPir, avgDistance, avgTilt, avgTemp, avgHum, avgLight);
      lastSendMs = now;
    }
  ...
  if (now - lastSendMs >= SEND_INTERVAL) {
    sendSensorPacket(false, false, secondsLeft, pir, distance, tilt, t, h, light);
    lastSendMs = now;
  }
